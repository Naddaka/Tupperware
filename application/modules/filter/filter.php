...
class Filter extends MY_Controller
{
    ...
    public function group($group_id = 0) {
        $form = $this->create_filter_form($group_id);
        if (!$form) {
        ...
        if ($this->input->post()) {
            $form->setAttributes($this->input->post());
        }
     ...
     * @return array|bool
     */
    public function search_items(array $fields = [], $type = 'page') {
        ...
        foreach ($query as $key => $field) {
            ...
            if (in_array($field['type'], ['select', 'checkgroup', 'radiogroup'])) {
