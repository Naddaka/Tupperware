 ...
 *
 * @method     ChildSCategoryI18nQuery leftJoinWith($relation) Adds a LEFT JOIN clause and with to the query
 * @method     ChildSCategoryI18nQuery rightJoinWith($relation) Adds a RIGHT JOIN clause and with to the query
 * @method     ChildSCategoryI18nQuery innerJoinWith($relation) Adds a INNER JOIN clause and with to the query
 *
 ...
 *
 * @method     ChildSCategoryI18nQuery joinWithSCategory($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the SCategory relation
 *
 * @method     ChildSCategoryI18nQuery leftJoinWithSCategory() Adds a LEFT JOIN clause and with to the query using the SCategory relation
 * @method     ChildSCategoryI18nQuery rightJoinWithSCategory() Adds a RIGHT JOIN clause and with to the query using the SCategory relation
 * @method     ChildSCategoryI18nQuery innerJoinWithSCategory() Adds a INNER JOIN clause and with to the query using the SCategory relation
 *
...
abstract class SCategoryI18nQuery extends ModelCriteria
{
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = SCategoryI18nTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]))))) {
    ...
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            SCategoryI18nTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
     ...
     * $query->filterByMetaDesc('%fooValue%', Criteria::LIKE); // WHERE meta_desc LIKE '%fooValue%'
     * </code>
     *
