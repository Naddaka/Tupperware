...
<form class="form"
      action="{site_url('comments/addPost')}#comments-anchor"
      method="post"
...
>
        ...
        <div class="star-voting__wrap">
          {for $i = 5; $i > 0; $i--}
            ...
            <label class="star-voting__icon"
                   for="star-voting-{$i}"
                   title="{$i} {tlang('out of 5 stars')}"><svg class="svg-icon svg-icon--star"><use xlink:href="{$THEME}_img/sprite.svg#svg-icon__star"></use></svg></label>
          {/for}
        </div>
      </div>
  ...
  <div class="form__field form__field--hor">
    ...
    <div class="form__inner">
      <button type="submit"
              class="btn btn-default">{tlang('Send')}</button>
    </div>
  </div>
  <!-- END Submit button -->
