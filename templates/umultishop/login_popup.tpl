<div class="modal modal--sm">
  ...
  <form action="{site_url('auth')}"
    ...
    <div class="modal__footer">
      <div class="modal__footer-row">
        <div class="modal__footer-btn hidden-xs">
          <button class="btn btn-default"
                  type="reset"
                  data-modal-close>{tlang('Close')}</button>
        </div>
      ...
      {if !$success}
        <div class="modal__footer-row">
          <div class="typo">{tlang("Don't Have an Account?")}<br><a class="form__link"
                                                                    href="{site_url('auth/register')}">{tlang('Create Account')}</a>
            / <a class="form__link"
                 href="{site_url('auth/forgot_password')}">{tlang('Forgot password?')}</a></div>
        </div>
      {/if}
    </div>
    ...
    {if strip_tags(trim($_REQUEST['wishlist']))}
      <input type="hidden"
             name="wishlist"
             value="{strip_tags(trim($_REQUEST['wishlist']))}"
             data-profile-wishlist="{strip_tags(trim($_REQUEST['wishlist']))}">
    {/if}
    {form_csrf()}
